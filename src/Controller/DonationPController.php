...
$result = $repo->findBy([], [$sortField => $sortDirection]); 
    ...
    public function ListDPFront(DonationPRepository $repo,SessionInterface $session): Response
    {
...
public function addFront(Request $request, ManagerRegistry $managerRegistry,SessionInterface $session): Response
{
          $userId = $session->get('user_id');
        ...
        $user = $managerRegistry->getRepository(Account::class)->find($accountId);

        $donationP = new DonationP();
        $donationP->setAccountKey($user);
...
//dd($form->isValid());
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('thankYouCard1', ['accountId' => $user, 
